...
def export_csv(company):
    ...
    try:
        ...
        # Company ke applicants filter karo
        company_apps = [a for a in applicants if a['company'] == company]
        company_apps.sort(key=lambda x: x['score'], reverse=True)
...
def submit_application():
    try:
        ...
        qualification = request.form.get('qualification')  # ✅ DIPLOMA BHI LEGA
        ...
        print(f"👤 Name: {name}")
        print(f"📧 Email: {email}")
        print(f"🎓 Qualification: {qualification}")  # ✅ DIPLOMA PRINT HOGA
        ...
        # Applicant data - ✅ QUALIFICATION SAVE HOGA
        applicant = {
            'id': timestamp,
            'name': name,
            'email': email,
            'qualification': qualification,  # ✅ YAHAN SAVE HOGA
            'company': company,
            'resume': filename,
